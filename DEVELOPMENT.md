...
*Zuletzt aktualisiert: 2026-07-27 — Item-Liste (Gesamtgewicht/-preis) sowie Verschieben und Löschen platzierter Komponenten direkt im Canvas ergänzt (Abschnitt 4 "Frontend"), inkl. eines dabei gefundenen, vorher nie ausgelösten Backend-Bugs im `MoveItem`-Endpoint (fehlendes `Category`-Include). Davor: Erster Teil des Maßstabs-Themas gelöst: `Component.RealWidthMm` (reales Referenzmaß, für alle 16 Seed-Komponenten befüllt) sorgt für relative Größenverhältnisse im Canvas statt fixer Pixelgrößen pro Rolle (Root/Kind). Der zweite, schwerere Teil (pixelgenaue geometrische Übereinstimmung zwischen unabhängigen Figma-Dateien inkl. Toleranzschwelle) bleibt bewusst offen, da er eine Maßstabs-Konvention für künftige Assets vom Projektinhaber braucht (Abschnitt 3). Der Canvas-Konfigurator deckt jetzt die komplette Kernvision aus Abschnitt 1 ab plus vollständige Item-Verwaltung (Hinzufügen, Verschieben, Löschen), nur eben noch mit den bisher zwei echten Assets.
