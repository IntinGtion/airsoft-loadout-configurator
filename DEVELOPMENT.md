...
**Dritter Gotcha, gefunden 2026-07-30:** Beim Aufräumen vor einem Commit fiel auf, dass der manuell re-exportierte `jpc-plate-carrier.svg` entgegen dem dokumentierten Workflow (Schritt 5 unten) noch alle 52 Marker-`<ellipse>`-Tags enthielt (`fill="#D9D9D9"`, identisch zur Silhouetten-Füllfarbe). Das fiel bei der visuellen Playwright-Verifikation des zweiten Gotchas nicht auf, weil die Ellipsen exakt dieselbe Füllfarbe wie der Träger-Körper haben und dadurch optisch komplett darin verschwinden (auch nach Colorway-Wechsel, da `recolorSvg.ts` alle `fill="#..."`-Vorkommen pauschal ersetzt, Ellipsen inklusive) — kein sichtbarer Bug, aber totes Gewicht in der Datei und ein Verstoß gegen das dokumentierte Rezept. Alle 52 `<ellipse>`-Zeilen wurden entfernt (`sed -i '/<ellipse /d'`), erneut mit Playwright verifiziert (alle 52 Stege weiterhin sauber sichtbar). **Lektion:** nach jedem manuellen Figma-Export künftig aktiv auf `<ellipse>`-Tags grep(p)en, nicht nur auf das visuelle Render-Ergebnis verlassen.
...
- **"My Loadouts"-Seite statt Dropdown** ✅ erledigt (2026-07-30) — das bisherige `LoadoutSwitcher`-Dropdown im Topbar (nur eine Namensliste) ersetzt durch eine eigene Route `/loadouts` (`LoadoutsPage`) mit einem Kachelraster: pro Loadout eine Karte mit Name, Item-Anzahl, Erstellungsdatum und einer echten **komponierten Thumbnail-Vorschau** (nicht nur ein Icon der Basis-Komponente)
