...
*Zuletzt aktualisiert: 2026-07-27 — Erster Teil des Maßstabs-Themas gelöst: `Component.RealWidthMm` (reales Referenzmaß, für alle 16 Seed-Komponenten befüllt) sorgt für relative Größenverhältnisse im Canvas statt fixer Pixelgrößen pro Rolle (Root/Kind) — sichtbar z.B. daran, dass die BFG-Pouch jetzt satt größer rendert und ungefähr die zwei MOLLE-Spalten abdeckt, die sie laut Footprint auch belegt. Der zweite, schwerere Teil (pixelgenaue geometrische Übereinstimmung zwischen unabhängigen Figma-Dateien inkl. Toleranzschwelle) bleibt bewusst offen, da er eine Maßstabs-Konvention für künftige Assets vom Projektinhaber braucht (Abschnitt 3). Damit sind Footprint-Matching, Drag & Drop, Colorway und die relative Größenskalierung alle vier fertig — der Canvas-Konfigurator deckt jetzt die komplette Kernvision aus Abschnitt 1 ab, nur eben noch mit den bisher zwei echten Assets.*
